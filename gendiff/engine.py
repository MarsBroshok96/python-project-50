...
import json
import copy
from gendiff.tree_constructor import (construct_1lvl_mutable_tree, get_node,
...
def generate_diff(file_path1, file_path2):
    ...
    #file1_keys = [key for key in file1.keys()]
    #file2_keys = [key for key in file2.keys()]
    ...
    added_tree = construct_1lvl_mutable_tree(dict(added_items), common_meta=ADDED_INFO)
    removed_tree = construct_1lvl_mutable_tree(dict(removed_items), common_meta=REMOVED_INFO)
    non_changed_tree = construct_1lvl_mutable_tree(dict(non_changed_items), common_meta=NON_CHANGED_INFO)

    diff_tree = copy.deepcopy(added_tree + removed_tree + non_changed_tree)

    def get_sorted(tree: list):

        result =  sorted(tree, key=lambda node: (get_name(node), get_meta(node)['origin']))
        return result
    ...
    def make_output_format(sorted_tree):
        def make_string_from_node(node):
            return f'{get_meta(node)["diff_status"]}{get_name(node)}: {get_value(node)}'
